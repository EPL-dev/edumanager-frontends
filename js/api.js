...
document.addEventListener('DOMContentLoaded', async function() {
  ...
  localStorage.setItem('edu_user', JSON.stringify(meRes.user));
  user = meRes.user;
...
async function updateDashboard() {
  ...
  if (!res.success) {
    showToast('Erreur chargement dashboard : ' + res.message, 'error');
    return;
  }
...
async function renderMyAttendance() {
  ...
  tbody.innerHTML = recs.map(function(r) {
    return '<tr><td>' + escHtml(r.subjectName || '–') + '</td><td>' + formatDate(r.date) + '</td>' +
...
function showToast(msg, type) {
  ...
  clearTimeout(t._t); t._t = setTimeout(function() { t.classList.add('hidden'); }, CONFIG.TOAST_DURATION || 3500);
}
